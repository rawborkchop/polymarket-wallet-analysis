...
class WalletViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def stats(self, request, pk=None):
        ...
        # Trade stats (always full range for totals)
        ...
        # Activity by type
        activities = wallet.activities.values('activity_type').annotate(
            count=Count('id'),
        ...
        from .pnl_calculator import calculate_wallet_pnl
        ...
        if chart_start or chart_end:
            ...
            for entry in daily_pnl_data:
                ...
                if chart_start:
                    try:
                        start_dt = datetime.strptime(chart_start, '%Y-%m-%d').date()
                        if date < start_dt:
                            continue
                    except ValueError:
                        pass
                if chart_end:
                    try:
                        end_dt = datetime.strptime(chart_end, '%Y-%m-%d').date()
                        if date > end_dt:
                            continue
                    except ValueError:
                        pass
        ...
        # ROI calculation
        total_bought = float(wallet.subgraph_total_bought or 0)
        ...
        if latest_analysis:
            analysis_metrics = {
                'win_rate_percent': float(latest_analysis.win_rate_percent) if latest_analysis.win_rate_percent else None,
                'profit_factor': float(latest_analysis.profit_factor) if latest_analysis.profit_factor else None,
                'max_drawdown_usd': float(latest_analysis.max_drawdown_usd) if latest_analysis.max_drawdown_usd else None,
                'subgraph_realized_pnl': float(latest_analysis.subgraph_realized_pnl) if latest_analysis.subgraph_realized_pnl else None,
                'buy_cost': float(latest_analysis.buy_cost) if latest_analysis.buy_cost else 0,
                'sell_revenue': float(latest_analysis.sell_revenue) if latest_analysis.sell_revenue else 0,
                'redeem_revenue': float(latest_analysis.redeem_revenue) if latest_analysis.redeem_revenue else 0,
        ...
        our_buys = float(trade_stats['total_volume'] or 0)
        coverage_pct = (our_buys / total_bought * 100) if total_bought > 0 else 0

        data = {
            'wallet': {
                'address': wallet.address,
                'name': wallet.name,
                'subgraph_realized_pnl': realized_pnl,
                'subgraph_total_positions': wallet.subgraph_total_positions or 0,
                'subgraph_total_bought': total_bought,
            ...
            'period_pnl': {
                ...
                'data_coverage_percent': round(coverage_pct, 1),
                'validation': pnl_result['validation'],
                'note': f"P&L for analyzed period. Subgraph shows lifetime P&L of ${realized_pnl:,.0f}"
            },
...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        # Top wallets by P&L
        top_wallets = Wallet.objects.exclude(
            subgraph_realized_pnl__isnull=True
        ).order_by('-subgraph_realized_pnl')[:5]
...
def refresh_wallet(request, pk):
    ...
    def do_refresh():
        try:
            _refresh_wallet_data(wallet, timeout_minutes=5)
        except Exception as e:
            print(f"Refresh error for {wallet.address}: {e}")
...
def extend_wallet_range(request, pk):
    ...
    Body: {"direction": "backward" | "forward", "days": 30}
    ...
    if start_date_str and end_date_str:
        ...
        return Response({'error': 'Provide direction (backward/forward) or start_date/end_date'}, status=status.HTTP_400_BAD_REQUEST)

    # Start background fetch for the new range
    def do_extend():
        try:
            _refresh_wallet_data(wallet, timeout_minutes=5, start_date=start_date, end_date=end_date)
        except Exception as e:
            print(f"Extend range error for {wallet.address}: {e}")
...
def _refresh_wallet_data(wallet, timeout_minutes=5, start_date=None, end_date=None):
    ...
    try:
        ...
        # Get accurate P&L from subgraph (with smaller page size for speed)
        ...
        # Update wallet P&L info and date range
